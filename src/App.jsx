...
const CLOUDINARY_CONFIG = {
  cloudName: 'gyzeubzm', // Substitua pelo seu Cloud Name se for diferente
  uploadPreset: 'album_uploads', // O nome do preset Unsigned que você criou
  folder: 'albums'
};
    ...
    (async function() { 
      try { 
        ...
        if (d) { 
          ...
          // PRÉ-CARREGAR O VÍDEO EM BACKGROUND (Ajustado)
          if (videoUrl) {
...
function AdminEditor(props) {
  ...
  // CORREÇÃO: Lógica de envio com Cloudinary sem duplicar imagens
  var hs = async function(e) { 
    ...
    try { 
      ...
      for (var i = 0; i < up.length; i++) { 
        var ph = up[i]; 
        // Não envia se já for uma string HTTP e não for um arquivo que acabou de ser lido (data/blob)
        if (typeof ph === 'string' && !ph.startsWith('data:') && !ph.startsWith('blob:')) { 
          urls.push(ph.startsWith('http') ? ph : 'https://' + ph); 
          step++; 
      ...
      if (smf) { 
        ...
        if (typeof fM === 'string' && fM.startsWith('blob:')) fM = ''; // Evita bug se apagar o áudio
        if (fM) step++;
      }
      ...
      if (videoFile) { 
        ...
        if (typeof fVideo === 'string' && fVideo.startsWith('blob:')) fVideo = ''; 
        if (fVideo) step++;
      }
